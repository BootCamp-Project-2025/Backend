...
@injectable()
export default class UpdateStudentTrackProgressUseCase implements IUseCase<StudentTrackProgress, void> {
    constructor(
    ...
    async execute(trackProgress: StudentTrackProgress): Promise<void> {
        try {
            const existing = await this.trackRepository.findById(trackProgress.id.toString());
            if (!existing)
                throw new ApiError(StatusCodes.NOT_FOUND, "StudentTrackProgress not found");
