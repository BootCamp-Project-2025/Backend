...
@injectable()
export class StudentTrackProgressRepository implements IStudentTrackProgressRepository {
    db = PrismaClient;
    async findById(id: string): Promise<StudentTrackProgress | null> {
        try {
            ...
            throw new ApiError(StatusCodes.INTERNAL_SERVER_ERROR, "Unknown error on database");
        }
    }
    ...
    async create(trackProgress: StudentTrackProgress): Promise<void> {
        try {
            const trackProgressData: StudentTrackProgressDb = StudentTrackProgressMapper.DomainToPersistence(trackProgress);
