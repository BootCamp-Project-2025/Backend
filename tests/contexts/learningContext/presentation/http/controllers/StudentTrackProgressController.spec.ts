...
describe("StudentTrackProgressController", () => {
    ...
    describe("create", () => {
        it("should create student track progress successfully", async () => {
            ...
            (StudentTrackProgressMapper.DtoToDomain as jest.Mock).mockReturnValue(mockTrackProgress);
            (mockStudentTrackService.create as jest.Mock).mockResolvedValue(undefined);

            const controller = makeController();
            await controller.create(mockReq, mockRes);

            expect(StudentTrackProgressMapper.DtoToDomain).toHaveBeenCalledWith(dto);
            expect(mockStudentTrackService.create).toHaveBeenCalledWith(mockTrackProgress, "enroll123");
        ...
        it("should throw ApiError when service fails", async () => {
            ...
            (StudentTrackProgressMapper.DtoToDomain as jest.Mock).mockReturnValue(mockTrackProgress);
            (mockStudentTrackService.create as jest.Mock).mockRejectedValue(new Error("Service error"));
        ...
        it("should rethrow ApiError from service", async () => {
            ...
            const controller = makeController();
            await expect(controller.create(mockReq, mockRes)).rejects.toThrow(apiError);
        });
    });
    ...
    describe("update", () => {
        it("should update student track progress successfully", async () => {
            ...
            (StudentTrackProgressMapper.DtoToDomain as jest.Mock).mockReturnValue(mockTrackProgress);
            (mockStudentTrackService.update as jest.Mock).mockResolvedValue(undefined);
            ...
            expect(mockStudentTrackService.update).toHaveBeenCalledWith(mockTrackProgress);
            expect(ResponseService.send).toHaveBeenCalledWith(
                mockRes,
                expect.objectContaining({
                    data: {},
        ...
        it("should throw ApiError when update fails", async () => {
            ...
            (StudentTrackProgressMapper.DtoToDomain as jest.Mock).mockReturnValue(mockTrackProgress);
            (mockStudentTrackService.update as jest.Mock).mockRejectedValue(new Error("Update error"));
    ...
    describe("getByEnrollment", () => {
        it("should get student progress by enrollment successfully", async () => {
            ...
            (mockStudentTrackService.getByEnrollment as jest.Mock).mockResolvedValue(mockStudentProgress);
            (StudentTrackProgressMapper.DomaintoDataStudentProgres as jest.Mock).mockReturnValue(mappedResult);

            const controller = makeController();
            await controller.getByEnrollment(mockReq, mockRes);

            expect(mockStudentTrackService.getByEnrollment).toHaveBeenCalledWith("enroll123");
            expect(StudentTrackProgressMapper.DomaintoDataStudentProgres).toHaveBeenCalledWith(mockStudentProgress);
            expect(ResponseService.send).toHaveBeenCalledWith(
                mockRes,
                expect.objectContaining({
                    data: mappedResult,
        ...
        it("should throw ApiError when fetching by enrollment fails", async () => {
            ...
            (mockStudentTrackService.getByEnrollment as jest.Mock).mockRejectedValue(new Error("Fetch error"));

            const controller = makeController();
            await expect(controller.getByEnrollment(mockReq, mockRes)).rejects.toThrow(
                new ApiError(
                    StatusCodes.INTERNAL_SERVER_ERROR,
    ...
    describe("getById", () => {
        it("should get student track progress by id successfully", async () => {
            ...
            (mockStudentTrackService.getById as jest.Mock).mockResolvedValue(mockTrackProgress);
        ...
        it("should throw ApiError when fetching by id fails", async () => {
            ...
            (mockStudentTrackService.getById as jest.Mock).mockRejectedValue(new Error("Fetch error"));
        ...
        it("should rethrow ApiError from service", async () => {
            ...
            (mockStudentTrackService.getById as jest.Mock).mockRejectedValue(apiError);

            const controller = makeController();
            await expect(controller.getById(mockReq, mockRes)).rejects.toThrow(apiError);
        });
    });
});