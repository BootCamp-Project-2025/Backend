...
const fakeTrackProgress = { lessonId: "lesson123", setAsCompleted: jest.fn(), isCompleted: jest.fn() } as unknown as StudentTrackProgress;
const fakeLesson = { props: { resources: [{ url: "a.pdf" }, { url: "b.png" }] } } as Lesson;
...
describe("StudentTrackProgressService", () => {
    ...
    describe("create", () => {
        it("should call createUseCase.execute with correct args", async () => {
            ...
            expect(createUseCase.execute).toHaveBeenCalledWith({ trackProgress: fakeTrackProgress, enrollmentId: "enroll1" });
        });

        it("should throw ApiError on unknown error", async () => {
            createUseCase.execute.mockRejectedValue(new Error("fail"));
            const service = makeService();
            await expect(service.create(fakeTrackProgress, "enroll1")).rejects.toThrow(ApiError);
        });
    });
    ...
    describe("getByEnrollment", () => {
        it("should return correct progress structure", async () => {
            ...
            getByEnrollmentUseCase.execute.mockResolvedValue([completed, notCompleted]);
            const service = makeService();
